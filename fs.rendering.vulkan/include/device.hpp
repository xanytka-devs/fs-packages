...
namespace vk_init {
	...
	bool is_suitable(const vk::PhysicalDevice& t_phys_device) {
		...
		if(check_device_ext_support(t_phys_device, requested_ext)) {
			firesteel::LOG_INFO("Device can support requested extensions.");
			return true;
		} else {
			firesteel::LOG_INFO("Device can't support requested extensions.");
			return false;
		}
	}
	...
	vk::PhysicalDevice get_device(vk::Instance& t_instance) {
		...
		firesteel::LOG_INFO("There are ", char(devices.size()) + " physical device(s) available on this system:");
	...
	vk::Device create_logical_device(vk::PhysicalDevice t_phys_device, vk::SurfaceKHR t_surface) {
		...
		try {
			...
			firesteel::LOG_INFO("GPU has been successfully abstracted.");
			return device;
		}
		catch (vk::SystemError err) {
			firesteel::LOG_CRIT("Device creation failed.");
			return nullptr;
		}
	}
